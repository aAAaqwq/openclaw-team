...
PINNACLE_MEANINGS = {
    1: '这是开创期。适合主动出击、建立个人品牌、换赛道、独立做决定。',
    2: '这是协同期。靠关系、伙伴、耐心和节奏感取胜，不宜硬冲。',
    3: '这是表达期。内容、传播、创意、社交、舞台感都会放大。',
    4: '这是筑基期。辛苦但值钱，适合打地基、建系统、做长期工程。',
    5: '这是变化期。迁移、转型、突破边界的机会多，但波动也大。',
    6: '这是责任期。家庭、团队、照顾他人、承担义务会变成主旋律。',
    7: '这是沉淀期。学习、研究、独处、修心比扩张更重要。',
    8: '这是收割期。事业、资源、权力、金钱议题会显著放大。',
    9: '这是完成期。旧周期需要收尾、放下、清理，为新阶段腾位置。'
}

CHALLENGE_MEANINGS = {
    0: '这段时期的课题不是单点困难，而是整体人生适应力。环境变化会逼你升级。',
    1: '课题是独立与自信。你要学会自己拍板，而不是总等别人给答案。',
    2: '课题是敏感与边界。你需要在理解别人和保护自己之间找到平衡。',
    3: '课题是表达与聚焦。避免情绪化和分散，把才华落到结果上。',
    4: '课题是纪律与稳定。面对限制时，关键不是抱怨，而是建立秩序。',
    5: '课题是自由与责任。变化很多，但不能因为想逃离就放弃承诺。',
    6: '课题是责任与控制欲。学会照顾别人，但不替别人活。',
    7: '课题是信任与孤独。不要把深度思考变成与世界隔绝。',
    8: '课题是权力与金钱。要学会既追求成就，也不被欲望反噬。',
    9: '课题是放下与慈悲。学会结束旧故事，不要反复消耗自己。'
}

RATIONAL_THOUGHT_MEANINGS = {
    1: '决策风格偏直接果断，倾向先定方向再微调细节。',
    2: '决策风格偏协商型，重视关系、反馈与环境气氛。',
    3: '思考方式带创意和表达欲，适合头脑风暴，但要防止发散。',
    4: '思考方式偏结构化，适合流程、系统、执行型问题。',
    5: '思考方式灵活机动，擅长快速适应，但容易频繁改主意。',
    6: '思考方式偏责任导向，做决定时会优先考虑长期影响和他人感受。',
    7: '思考方式偏分析和求证，适合复杂判断，但容易过度思考。',
    8: '思考方式偏结果导向，强调整体盘面、资源配置与现实回报。',
    9: '思考方式偏全局主义，容易站在意义、价值和长周期上看问题。',
    11: '思考方式带强直觉和洞察，适合非线性判断，但要避免神经过载。',
    22: '思考方式兼具蓝图和落地，适合做大系统规划与现实整合。',
    33: '思考方式带教练型和疗愈型倾向，善于理解复杂的人性议题。'
}
...
class NumerologyInterpreter:
    ...
    def get_combined_reading(self, full_name: str, year: int, month: int, day: int,
        ...
        rt_val = snap.get('rational_thought')
        pinnacles = report.get('pinnacles', {})
        ...
        rational_text = self._interpret_rational_thought(rt_val)
        pinnacle_text = self._interpret_pinnacles_and_challenges(pinnacles, challenges)
        ...
        return {
            ...
            'rational_thought_reading': rational_text,
            'life_cycles_reading': pinnacle_text,
    ...
    def _interpret_rational_thought(self, val: Optional[int]) -> str:
        if not val:
            return '理性思维数暂不可用。'
        v = val if val in RATIONAL_THOUGHT_MEANINGS else _reduce_number(val, False)
        return f'【理性思维数】{val}号：{RATIONAL_THOUGHT_MEANINGS.get(v, "你的决策方式偏综合型，需要结合情境判断。")} '

    def _interpret_pinnacles_and_challenges(self, pinnacles: Dict, challenges: Dict) -> str:
        lines = []
        order = [('first', '第一阶段'), ('second', '第二阶段'), ('third', '第三阶段'), ('fourth', '第四阶段')]
        for key, label in order:
            p = pinnacles.get(key, {})
            c = challenges.get(key, {})
            pv = p.get('value')
            cv = c.get('value')
            age = p.get('age_range') or c.get('age_range', '')
            p_text = PINNACLE_MEANINGS.get(pv, '这是一个需要顺势理解的大周期。')
            c_text = CHALLENGE_MEANINGS.get(cv, '这段时间的课题需要你在实践里慢慢体会。')
            lines.append(f'【{label} {age}】顶峰数{pv}：{p_text} 挑战数{cv}：{c_text}')
        return '\n'.join(lines)
