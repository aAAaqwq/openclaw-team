...
# 大六壬
try:
    from daliuren_scores import score_daliuren_dimensions as _daliuren_scores
...
DEFAULT_CALIBRATION_OFFSETS = {
    "八字": {"事业": 1.7, "财运": -1.15, "健康": -2.4, "婚姻": -2.55, "人际": -1.15},
    "紫微": {"事业": 2.7, "财运": 1.0, "健康": 0, "婚姻": -0.7, "人际": 0.4},
    "奇门": {"事业": 2.75, "财运": 0.95, "健康": 0, "婚姻": -3.1, "人际": 0.75},
    "六爻": {"事业": 0.0, "财运": 0.0, "健康": 0.0, "婚姻": 0.0, "人际": 0.0},
    "大六壬": {"事业": -1.2, "财运": -1.0, "健康": -1.0, "婚姻": -0.5, "人际": -0.8},
    "小六壬": {"事业": 2.5, "财运": 4.0, "健康": 4.0, "婚姻": 3.5, "人际": 2.5},
}
...
def analyze_bazi(year: int, month: int, day: int, hour: int = 12,
    ...
    try:
        ...
        dimensions = score_bazi_dimensions(result)
    except Exception:
        dimensions = {}
        for dim in DIMENSION_KEYS:
            dim_score = min(10, score // 10 + 2) if dim == "事业" else 6
            dimensions[dim] = {"score": dim_score, "level": "高" if dim_score >= 7 else ("中" if dim_score >= 4 else "低")}
...
def analyze_daliuren(year: int, month: int, day: int, hour: int = 12) -> Dict:
    ...
    try:
        ...
        if scores.get("available"):
            return {
                "available": True,
                "result_summary": scores.get("result_summary", ""),
                "dimensions": scores.get("dimensions", {}),
                "trend": {"trend_text": scores.get("result_summary", "大六壬分析完成")},
            }
    except Exception as e:
        return {"available": False, "error": str(e)}
